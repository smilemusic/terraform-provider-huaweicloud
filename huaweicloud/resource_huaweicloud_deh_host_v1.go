...
func resourceDeHHostV1Create(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("Error creating HuaweiCloud Dedicated Host: %s", err)
	}
	d.SetId(allocate.AllocatedHostIds[0])

	log.Printf("[DEBUG] Waiting for HuaweiCloud Dedicated Host (%s) to become available", allocate.AllocatedHostIds[0])
